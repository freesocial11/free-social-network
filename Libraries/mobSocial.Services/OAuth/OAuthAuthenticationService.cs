...
namespace mobSocial.Services.OAuth
{
    public class OAuthAuthenticationService : AuthenticationService
    {
        ...
        {
            var ctx = HttpContext.Current.GetOwinContext();
            ctx.Authentication.SignOut("Application");
            ...
            ctx.Response.Cookies.Append(".AspNet.Application", "", new CookieOptions()
            {
        ...
        public override User GetCurrentUser()
        {
            ...
            _user = _userService.GetCompleteUser(emailClaim.Value) ?? base.GetCurrentUser();
